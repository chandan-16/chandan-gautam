...
const Navbar = ({ isDarkMode, setIsDarkMode }) => {
  ...
  return (
    ...
    <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? "bg-opacity-50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20 " : ""}`} >
        <a href="#top">
            <h1 className="text-gray-700 font-semibold text-3xl text-left align-middle dark:text-white">Gautam <span className="text-red-600 font-bold text-4xl -pr-[5px]">.</span></h1>
        </a>

        <ul className={`hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 ${isScroll ? "" : "bg-white shadow-sm bg-opaciy-50 dark:border dark:border-white/50 dark:bg-transparent" }`}>
            <li><a href="#top" className="font-Ovo">Home</a></li>
            <li><a href="#about" className="font-Ovo">About me</a></li>
            <li><a href="#services" className="Ovo">Services</a></li>
    ...
    <ul ref={sideMenuRef} className="flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 dark:bg-darkHover dark:text-white">
        ...
        <li><a href="#services" className="Ovo" onClick={closeMenu}>Services</a></li>
